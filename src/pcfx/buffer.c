...
const uint16_t pornpal[256] =
{
	//0x79A7, 0x57A7, 0x35A7, 0x0088, 0xA687, 0xD987, 0x7387, 0x5187, 0x8C6A, 0xC16A, 0xDB79,
	0x88, 0x88, 0x88, 0x88, 0x88, 0x88, 0x88, 0x88, 0x88, 0x88, 0x88, 
	
	0x8732, 0x6943, 0x7543, 0x9531, 0xda0a, // Used by game 
...
void bufDisplay(void *buffer)
{
	...
	eris_king_set_kram_read(0xA00, 2);
	eris_king_set_kram_write(0xA00, 2);
	// Clear BG0's RAM
	#ifdef _16BPP
	for(i = 0x0; i < SCREEN_BYTES_SIZE/2; i++) {
...
void bufHorizLine(void *buffer,int x,int y,int x2, int color)
{
	...
	if(x2>=0 && x<SCREEN_GAME_WIDTH && y>=0 && y<SCREEN_GAME_HEIGHT)
	{
		...
		memset(buffer + x + y * SCREEN_GAME_WIDTH, color, x2 - x);
		/*for(i=x;i<=x2;i++)
		{
